...
ListModel {
    ...
    function getIndexByAttributes (attrs) {
        // Only for objects with values that are strings, numbers, booleans
        // E.g. attrs = {"name": "Jake", "number": 1, "check": true}
        var attrKeys = []
        for (var k in attrs) attrKeys.push(k)
    ...
    function getItemsByAttributes (listModel, attrs) {
        // Only for objects with values that are strings, numbers, booleans
        // E.g. attrs = {"name": "Jake", "number": 1, "check": true}
        var attrKeys = []
        for (var k in attrs) attrKeys.push(k)
        for (var i=0; i<count; i++) {
            ...
            if (allAttributesPresentInItem) listModel.append(item);
        }
    }
