...
QtControls.Page {
    ...
    property alias moreMenu:unitspopup
    readonly property url units_select:"../../MapViewer/images/check.png"
    property  var selectedMeasurementUnits:mapView.selectedMeasurementUnits
    ...
    onSelectedMeasurementUnitsChanged: {
        ...
        if(selectedMeasurementUnits === measurementUnits.imperial)
            setTitleWithUnits(strings.ft)
        else
            setTitleWithUnits(strings.m)
    }

    onScreenWidthChanged: {
        unitspopup.x = app.isRightToLeft ? x + app.units(6) :x - unitspopup.width + app.units(26)
    }
    ...
    function getXY(measure,startPoint)
    {
        ...
        if(deltaDistance > 0)
        {
            ...
            feature = mapView.identifyProperties.features[mapView.currentFeatureIndexForElevation]
    ...
    function drawElevationChart()
    {
        ...
        if(mapView.currentFeatureIndexForElevation > -1){
            ...
            if(selectedMeasurementUnits === measurementUnits.metric)
                trailLength  = ((_trailLength / 1000).toFixed(2)).toString() + strings.km//"Km"
            else
                trailLength  = (_trailLength * metersToMiles).toFixed(2).toString() + strings.mi//"Mi"
    ...
    //if user is signed in we pass the token and query the mail_service_not_configured
    //if user is not signed in we derive the summary from chart data.
    ...
    function fetchSummary(feature)
    {
        ...
        var _inFeatures = {
            ...
            //features: [{geometry: feature.geometry.json}]
        }
                ...
                ChartView {
                    ...
                    MouseArea{
                        ...
                        onClicked:{
                            ...
                            /*  if((distanceArray[distanceArray.length - 1] - cp.x) < 0.005)
                            {
                                cp.x = distanceArray[distanceArray.length - 1]
                                xdistance = cp.x
                                targetPoint = Qt.point(cp.x, cp.y)
                            }*/
                            ...
                            if(selectedMeasurementUnits === measurementUnits.imperial)
                                distanceInMeters = cp.x /metersToMiles  //+ 100.00
                            let startPoint = mapLocationDic[distanceArray[0]]//mapLocationDic["0.0000"]

                            let mapPointObject = getXY(cp.x,startPoint)//getXY(distanceInMeters,startPoint)
...
*/
                    ...
                    ValueAxis {
                        ...
                        min: mMin
                        max: mMax
                        titleText: selectedMeasurementUnits === measurementUnits.imperial ? strings.distance_units.arg(strings.mi):strings.distance_units.arg(strings.km)
            ...
            RowLayout{
                ...
                Rectangle{
                    ...
                    GridView {
                        ...
                        layoutDirection: Qt.LeftToRight//Text.AlignLeft
    ...
    QtControls.Popup{
        id:unitspopup
        width: 200
        height: menuCol.height + app.units(20)
        margins: 0
        ...
        ColumnLayout{
            ...
            ListView {
                ...
                Layout.preferredHeight: unitsModel.count * app.units(25) + app.units(30)
                ...
                delegate:Item{
                    ...
                    RowLayout{
                        ...
                        Item {
                            ...
                            QtControls.RadioButton {
                                ...
                                onClicked: {
                                    ...
                                    mapView.selectedMeasurementUnits = value
                        ...
                        Controls.SubtitleText {
                            id: lbl
                            objectName: "label"
                            Layout.preferredWidth: parent.width - radioButton.width - Layout.rightMargin
                            Layout.fillHeight: true
